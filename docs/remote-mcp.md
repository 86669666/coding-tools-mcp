...
context. Command runs are workspace resources instead: `exec_command` returns a
`run_id`, and another authenticated client connected to the same workspace can
use that ID with `write_stdin`, `read_output`, or `kill_session`. The legacy
`session_id` field remains accepted as an alias. `DELETE /mcp` terminates only
the selected transport runtime; it does not terminate workspace runs. HTTP
