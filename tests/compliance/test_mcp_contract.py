...
class MCPContractTests(ComplianceTestCase):
    ...
    def test_http_sessions_isolate_cwd_but_share_workspace_runs(self) -> None:
        ...
        with MCPClient(self.workspace.root, url=self.client.url) as sibling:
            ...
            self.assertEqual(payload.get("session_id"), run_id)
            polled = self.assert_tool_success(
                sibling.call_tool("write_stdin", {"run_id": run_id, "chars": "", "yield_time_ms": 0})
            )
            self.assertEqual(polled.get("run_id"), run_id)
            sibling.call_tool("kill_session", {"run_id": run_id, "signal": "KILL"})
    ...
    def test_http_session_delete_does_not_terminate_workspace_run(self) -> None:
        ...
        polled = self.assert_tool_success(
            self.client.call_tool("write_stdin", {"run_id": run_id, "chars": "", "yield_time_ms": 0})
        )
        self.assertEqual(polled.get("status"), "running")
        killed = self.assert_tool_success(
            self.client.call_tool("kill_session", {"run_id": run_id, "signal": "KILL"})
        )
